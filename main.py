...
parser.add_argument('--hidden_size', type=int, default=8, metavar='N',
...
LSIZE = 64  # SAC latent input size (concatenated the outputs of VAE and MDRNN)

# Start the Duckietown gym env and initialize the action, reward and observation wrappers
env = launch_env(map_name = training_map[10])
env = wrap_env(action_type = action_type[0], env = env, add_observation_wrappers = True, add_action_wrappers = True, add_reward_wrappers = True, lane_penalty = False)
...
for i_episode in itertools.count(1):
    ...
    while not done:
        ...
        episode_steps += 1
        total_numsteps += 1
        print('total_numsteps: {}'.format(total_numsteps))
        print('args_numsteps: {}'.format(args.num_steps))
